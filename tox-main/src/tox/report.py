...
try:
    ...
    class _FakeColorama:
        RED = CYAN = WHITE = GREEN = MAGENTA = BRIGHT = NORMAL = DIM = RESET_ALL = ""
...
class _LogThreadLocal(local):
    ...
    def __init__(self, out_err: OutErr) -> None:
        parent_ident = getattr(current_thread(), "parent_ident", None)
        self._name = self._ident_to_data.get(parent_ident, "ROOT")
...
class ToxHandler(logging.StreamHandler):  # type: ignore[type-arg]
    """Controls tox output."""
